...
- Play enforces a target-API level annually (deadline ~Aug 31): targetSdk must stay within one year of the latest Android. Keep `targetSdk` = `compileSdk` at the newest stable API to avoid the "update your target API level" block; bumping it needs a versionCode bump + release rebuild, and verify the bump landed via `aapt2 dump badging` (targetSdkVersion), not just a green build.
